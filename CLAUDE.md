...
`search()` returns one `FileResult` per matching file with `score = (titleMatch ? 1 : 0) + matches.length` and `mtime` (last-modified, ms since epoch — `FileInput.mtime`, populated by `handleSearch` via `vscode.workspace.fs.stat`, alongside the `fs.readFile` call, per file). `searchViewProvider.handleSearch` sorts by filename, score, or `mtime` (ascending or descending) depending on the webview's sort selector (`'name' | 'relevance' | 'date-asc' | 'date-desc'`), but in all four modes files whose **filename** matches the query (`titleMatch`) always sort before files that only matched in the body — that's the primary sort key, the chosen mode is just the tiebreaker. Highlights are built with a ±60-char context window (`CONTEXT_RADIUS`) per match in `buildContext`.
...
`searchEngine.search()` is `async`: every `YIELD_EVERY_FILES` (20) files it awaits a `setImmediate` and checks an optional `AbortSignal`, throwing `SearchCancelledError` if aborted. This exists because a too-generic query (e.g. a single common letter) over a large vault can otherwise block the extension host's event loop for several seconds of synchronous scanning — with no yield point, a cancel request could never even be received, let alone acted on. `searchViewProvider.handleSearch` keeps a `currentSearch: AbortController`, aborting the previous one at the start of every new search (superseding it, whether the new one came from another keystroke or the cancel button) and swallowing `SearchCancelledError` without posting `'results'`. The webview's `#loadingIndicator` (shown after `LOADING_DELAY_MS`, same as the spinner) includes a "Cancelar" button (`main.js`'s `cancelSearch()`) that hides the indicator, shows a "Búsqueda cancelada." empty state, and posts `cancelSearch` to the host immediately — it doesn't wait for the host's abort to complete. A `searchCancelled` flag set by that click (cleared at the start of the next `runSearch()`) makes the `'results'`/`'error'` message handler discard a late response from the just-cancelled search, in case it was already in flight back from the host when the cancel message went out.

### Focusing the search box on view activation

Selecting the panel (activity bar icon, or the `obsidianlikeSearch.open` command) must always land focus on `#searchInput`. `resolveWebviewView` requests focus as soon as the view is resolved, and also subscribes to `webviewView.onDidChangeVisibility` to re-request it on every later reveal (a `WebviewView` in the sidebar is resolved once and then just toggles visibility — it doesn't re-fire `resolveWebviewView`/the webview's `ready` message on subsequent selections). Both paths go through `sendFocus()`, which mirrors the `pendingQuery`/`runQuery` pattern: a `postMessage({ command: 'focus' })` sent before the webview script has attached its `message` listener (still loading on first activation) is silently dropped, so `sendFocus()` queues a `pendingFocus` flag instead and flushes it once the webview's `ready` message arrives. `focusInput()` (used by the `obsidianlikeSearch.open` command) goes through the same `sendFocus()` for the same reason. `media/main.js`'s `message` listener handles `'focus'` by calling `searchInput.focus()` + `.select()`.
...
- Search re-reads and re-scans every matching file on each query (no index/cache); fine for typical vault sizes, would need revisiting for very large vaults. A too-generic query on a large vault is cancellable (see "Cancelling an in-flight search") rather than blocking indefinitely, but the scan itself is still brute-force.
